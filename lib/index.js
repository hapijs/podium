...
internals.Podium.prototype.emit = function (criteria, data, callback) {
    ...
    return this._emit(criteria, data, callback);
};


internals.Podium.prototype._emit = function (criteria, data, callback) {
...
internals.distribute = function (emitter, notification) {
    ...
    const finalize = notification.callback || Hoek.ignore;

    const callback = function () {

        const each = (podium, next) => podium._emit(criteria, data, next);      // User _emit() in case emit() was modified
...
internals.emit = function (emitter, notification) {
    ...
    const each = (item, next) => {
        ...
        const eachHandler = (handler, nextHandler) => {
            ...
            const invoke = (func) => {
                ...
                if (typeof data === 'function' &&
                    data === item.data) {                   // Prevent invoking function data returned from function data
        ...
        Items.parallel(handlers.slice(), eachHandler, nextItem);        // Clone in case handlers are changed by listeners
    };
