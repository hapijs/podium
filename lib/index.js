...
internals.Podium.prototype.registerEvent = function (events) {
    ...
    events.forEach((event) => {
        ...
        Joi.assert(event, internals.schema.event, 'Invalid event options');
    ...
    const each = (item, next) => {
        ...
        const eachHandler = (handler, nextHandler) => {
            ...
            const invoke = (func) => {
                ...
                const args = (internals.flag('spread', handler, event) ? data : [data]);
...
internals.Podium.prototype.on = internals.Podium.prototype.addListener = function (criteria, options, listener) {
    ...
    if (options.filter &&
        ...
        options.filter = { tags: options.filter };
    }

    Joi.assert(options, internals.schema.listener, 'Invalid event listener options');
