    ...
    const each = (item, next) => {
        ...
        const eachHandler = (handler, nextHandler) => {
            ...
            const invoke = (func) => {
                ...
                const data = (internals.flag('clone', handler, event) ? Hoek.clone(item.data) : item.data);
                const args = (internals.flag('spread', handler, event) && Array.isArray(data) ? data : [data]);
