...
internals.Podium.prototype.emit = async function (criteria, data) {

    return new Promise((resolve, reject) => {

        try {
            this._emit(criteria, data, false, resolve);
        }
        catch (err) {
            reject(err);
        }
...
internals.Podium.prototype.once = function (criteria, listener) {
    ...
    if (!listener) {
        override.spread = false;
    }

    criteria = Object.assign(internals.criteria(criteria), override);

    if (listener) {
        return this.on(criteria, listener);
    }

    return new Promise((resolve) => this.on(criteria, resolve));
};
