...
internals.Podium.prototype._emit = function (notification) {
    ...
    const each = (item, next) => {
        ...
        const eachHandler = (handler, nextHandler) => {
            ...
            if (handler.callback) {
                return handler.listener(item.data, nextHandler);
            }
        ...
        let handlers = this._subscribers[item.event];
        handlers = (handlers ? handlers.slice() : []);              // Clone in case handlers are changed by listeners

        Items.serial(handlers, eachHandler, () => {
