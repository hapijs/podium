...
internals.Podium.prototype.emit = function (criteria, data) {

    return new Promise((resolve) => this._emit(criteria, data, false, resolve));
};
...
internals.emit = function (emitter, notification) {
    ...
    const relay = () => {
        ...
        const each = (podium, next) => podium._emit(item.criteria, data, generated, next);        // Use _emit() in case emit() was modified
